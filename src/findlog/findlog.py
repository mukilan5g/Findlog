...
def get_files_in_dir(path):
    ''' This function takes the path and
        return only the list files in that leaf directory.
...
def search_files_by_re(file):
    ''' This function takes the full path of the file.
        If the file name is a regular expression,
        ...
        note:
        Always specify the path with '\\',don't use '\'.
        Because the function won't read.
        It will throw this error:
        {WindowsError: [Error 123] The filename, directory name,
...
def get_files_match_any(patterns):
    '''This function takes the list of patterns and return the files
       which contains the contents matches those patterns anywhere in the file.
       It will return all files if any one of the pattern
       matches with the file contents.
       ...
       {'^w': ['C:\\Users\\Radhu\\Desktop\\webcrawl1.py']})
    '''
    list_of_files = {}
    for pattern in patterns:
        list_of_files[pattern] = []
    for pattern in patterns:
        for file_pattern in (open(r'__paste the path here__\.files',
                                  'r').read()).split('\n'):
...
def get_files_match_all(patterns):
    '''This function takes the list of patterns and return the files
       which contains the contents matches those patterns anywhere in the file.
       It will return all files only if all the pattern matches
